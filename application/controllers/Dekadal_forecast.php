...
class Dekadal_forecast extends CI_Controller
{
    ...
    {    
        $dekadal_forecast_data = $this->Decadal_forecast_model->get_all(); 
       // print_r($dekadal_forecast_data);exit();dekadal_forecast_data
    ...
    function saveForecast(){
        ...
        $date_from = strtotime($this->input->post('date_from',TRUE));
        $date_from_time = date('Y',$date_from).'-'.date('m',$date_from).'-'.date('d',$date_from);
        $date_to = strtotime($this->input->post('date_to',TRUE));
        $date_to_time = date('Y',$date_to).'-'.date('m',$date_to).'-'.date('d',$date_to);
       

    $datatoinsert = array(
                'date_from' => $date_from_time,
                'date_to' => $date_to_time,
                'issue' => $date_time,
             'general_info' => $this->input->post('general_information',TRUE),
      ...
      //====== checking the presence of $id==========
        if (!is_null($id) && $this->Decadal_forecast_model->get_by_id($id)) {
            $this->Decadal_forecast_model->update_by_id($id,$datatoinsert);
        } else {
		$this->Decadal_forecast_model->insert($datatoinsert);
        }  	
    ...
    //change messageluganda
         $data = array(
            'change' => 87,
            'advisory_read' => $this->Decadal_forecast_model->get_it_by_id_replaced($id),
            'rem' => "show"
        );
        $this->load->view('template', $data);
    ...
    public function create_advisory()
    {
        ...
        $this->Decadal_forecast_model->insert_advisory($datatoinsert);
        $data = array(
            'change' => 82,
            'get_all_advisory' => $this->Decadal_forecast_model->get_all_advisory()
        );          
...
//var_dump($farm);exit;
	...
	public function save(){
     ...
     {
        $id =  $this->uri->segment(3);
       $data['dekadal_data'] = $this->Decadal_forecast_model->get_data_by_id($id);
     $data['change'] = 71;        
     $this->load->view('template', $data);
    }
