...
class Season_model extends CI_Model
{
    ...
    {   
        $this->db->select('division.division_name, division.division_type,region.region_name, area_seasonal_forecast.expected_peak, area_seasonal_forecast.peakdesc, area_seasonal_forecast.onset_period, area_seasonal_forecast.onsetdesc, area_seasonal_forecast.end_period, area_seasonal_forecast.enddesc, area_seasonal_forecast.overall_comment, area_seasonal_forecast.general_info ,seasonal_forecast.overview, seasonal_forecast.year, seasonal_forecast.general_forecast, seasonal_forecast.created, seasonal_forecast.issuetime,  seasonal_forecast.map,season_months.season_name, season_months.month_from, season_months.month_to, season_months.abbreviation, sub_region.sub_region_name');
...
public  function get_forecast_area($id=NULL){
      ...
      $this->db->join('region','region.id=area_seasonal_forecast.region_id','inner'); 
     if(isset($id)){
	   $this->db->where('area_seasonal_forecast.region_id',$id);	 
	}
     $query=$this->db->get();   
      return $query->result_array();
  ...
  // get total rows
    ...
    function get_season_area_data($id, $division)
    {  
        $this->db->select('division.division_name, division.division_type,region.region_name, area_seasonal_forecast.expected_peak, area_seasonal_forecast.peakdesc, area_seasonal_forecast.onset_period, area_seasonal_forecast.onsetdesc, area_seasonal_forecast.end_period,seasonal_forecast.map, area_seasonal_forecast.enddesc, area_seasonal_forecast.overall_comment, area_seasonal_forecast.general_info ,seasonal_forecast.overview, seasonal_forecast.year, seasonal_forecast.general_forecast, seasonal_forecast.created, seasonal_forecast.issuetime,season_months.season_name, season_months.month_from, season_months.month_to, season_months.abbreviation, sub_region.sub_region_name');
        $this->db->from('division');
        $this->db->join('region','division.region_id = region.id');
        $this->db->join('area_seasonal_forecast','area_seasonal_forecast.region_id =  division.region_id');
        $this->db->join('sub_region','area_seasonal_forecast.subregion_id = sub_region.id');
