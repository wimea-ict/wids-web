  ...
  $user_feedback = $this->user_feedback_model->get_all();
    ...
    public function readfeedback(){
      //  $feedback = $this->user_feedbak_model->readfeedback();
        $data['user_feedbacks'] = $this->lang->line('msg_user_feedbacks');
        $data['category'] = $this->lang->line('msg_category');
        $data['district_name'] = $this->lang->line('msg_district_name');
        $data['number'] = $this->lang->line('msg_number');
        $data['change'] = 40;
        $this->load->view('template', $data);
    }
    ...
    public function read_feedback($feedbackid){
...
$this->load->view('template', $data);
    ...
    public function log_userfeedback(){
      ...
      $Info = $this->user_feedback_model->logfeedback($data);
      if ($Info) {
        ?>
        <script>
    ...
    public function feedback_pdf(){
        ...
        $html = $this->load->view('read_user_feedback_pdf', $data, true);
        $this->load->library('pdf');
        $pdf = $this->pdf->load();
        $pdf->WriteHTML($html);
        $pdf->Output('user_feedback.pdf', 'D'); 
